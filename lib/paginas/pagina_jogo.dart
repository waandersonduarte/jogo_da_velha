...
class _GamePageState extends State<GamePage> {
  ...
  _checkWinner() {
    ...
    if (winner == TipoVencedor.none) {
      if (!_controller.hasMoves) {
        _showTiedDialog();
      } else if (_controller.isSinglePlayer &&
          _controller.currentPlayer == TipoJogador.player2) {
        final index = _controller.automaticMove();
        _onMarkTile(index);
      }
    } else {
      String symbol =
          winner == TipoVencedor.player1 ? PLAYER1_SYMBOL : PLAYER2_SYMBOL;
  ...
  _buildPlayerMode() {
    return SwitchListTile(
      title:
          Text(_controller.isSinglePlayer ? 'Único Jogador' : 'Dois Jogadores'),
